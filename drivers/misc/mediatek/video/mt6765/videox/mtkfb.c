...
/* Zhijun.Ye@MM.Display.LCD, 2021/04/06, add for local dre support */
#endif
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	init_state = 0;
#ifdef OPLUS_BUG_STABILITY
/*Xiaoqi.Hao@PSW.MM.Display.LCD.Machine ,2022/01/15, zhaoyun get the normal max brightness*/
...
/* wangcheng@ODM.Multimedia.LCD  2020/10/09 add for lcd shutdown power control  end */
	...
	/* zengshangwen@ODM.Multimedia.LCD  2021/7/7 add for lcd shutdown power control	start */
#ifdef OPLUS_BUG_STABILITY
	primary_display_shutdown();
#endif
	/* zengshangwen@ODM.Multimedia.LCD  2021/7/7 add for lcd shutdown power control	end */
