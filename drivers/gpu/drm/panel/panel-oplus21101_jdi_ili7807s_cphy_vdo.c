...
/*#ifdef OPLUS_TP_FEATURE_BASIC shifan@bsp.tp 2021.0415*/
...
/*#endif OPLUS_TP_FEATURE_BASIC shifan@bsp.tp 2021.0415*/
...
/*#ifdef OPLUS_TP_FEATURE_BASIC shifan@PSW.BSP.TP.Function, 2019/04/26, Add for TP black gesture*/
	pr_info("%s: tp_gesture_enable_flag = %d \n", __func__, tp_gesture_enable_flag());
	if (0 == tp_gesture_enable_flag() || (esd_flag == 1) /*|| (shutdown_flag == 1)*/) {
/*#endif OPLUS_TP_FEATURE_BASIC shifan@PSW.BSP.TP.Function, 2019/04/26, Add for TP black gesture*/
...
/*#ifdef OPLUS_TP_FEATURE_BASIC shifan@bsp.tp 2021.0402 add for leaking current from TP's gpio*/
		tp_gpio_current_leakage_handler(false);
/*#endif OPLUS_TP_FEATURE_BASIC shifan@bsp.tp 2021.0402 add for leaking current from TP's gpio*/
...
/*#ifdef OPLUS_FEATURE_TP_BASIC shifan@bsp.tp, 2021.0411, add for touchpanel power on gpio operations and loading fw*/
	if (tp_boot_mode_normal()) {
		tp_gpio_current_leakage_handler(true);
		lcd_queue_load_tp_fw();
	}
/*#endif OPLUS_FEATURE_TP_BASIC shifan@bsp.tp, 2021.0411, add for touchpanel power on gpio operations and loading fw*/
	lcm_panel_init(ctx);
	ret = ctx->error;
	if (ret < 0)
		lcm_unprepare(panel);
