...
#ifdef OPLUS_BUG_STABILITY
/* gaoxiaolei@PSW.MM.DisplayDriver.FP, 2021/02/23, optimize cmdq timeout log*/
...
/*#endif*/
/*#ifdef OPLUS_BUG_STABILITY*/
/*Yaqiang.Shi@RM.Display.LCD.Driver, add for eyeprotect*/
...
int mtk_drm_setbacklight(struct drm_crtc *crtc, unsigned int level)
{
	...
	bool is_frame_mode;
	#ifdef OPLUS_BUG_STABILITY
	/* shangruofan@PSW.MM.feature.onfingerprint, 2021/01/05, add for forbidden backlight when hbm is on */
	bool state = false;
	#endif
	int index = drm_crtc_index(crtc);
...
/*liwei.a@PSW.MM.LCD.Display.Stability, 2020/12/30, add for seed*/
...
int mtk_drm_aod_setbacklight(struct drm_crtc *crtc, unsigned int level)
{
	...
	/* Zhijun.Ye@PSW.MM.LCD, 2022/04/20, delete for aod */
	bool is_frame_mode;
	#endif
	...
	/* send LCM CMD */
	#ifndef OPLUS_BUG_STABILITY
	/* Zhijun.Ye@PSW.MM.LCD, 2022/04/20, modify for aod */
	is_frame_mode = mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base);
...
#ifdef OPLUS_BUG_STABILITY
	/* chenjun@PSW.MM.Display.LCD.Machine 2021/11/11, add for hbm flicker */
	if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
...
static void mtk_crtc_cmdq_timeout_cb(struct cmdq_cb_data data)
{
	...
	if ((mtk_crtc->trig_loop_cmdq_handle) &&
		...
		#ifdef OPLUS_BUG_STABILITY
		/* gaoxiaolei@PSW.MM.DisplayDriver.Stability, 2021/02/22, optimize cmdq timeout log*/
...
#endif
{
	...
	for_each_comp_in_cur_crtc_path(comp, mtk_crtc, i, j) {
#ifdef OPLUS_BUG_STABILITY
	/* chenjun@PSW.MM.Display.LCD.Machine 2021/12/10, fix bypass aal,service write old backlight */
				      ...
				      struct drm_crtc_state *old_crtc_state)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_HBM)) {
		...
		if (!state->prop_val[CRTC_PROP_DOZE_ACTIVE]) {
			#ifdef OPLUS_BUG_STABILITY
			/* chenjun@PSW.MM.Display.LCD.Machine 2021/11/29, fix bypass pq invalid para*/
			int bypass_pq_en = 0;
			if (hbm_en == 0)
				bypass_pq_en = 0;
			else
